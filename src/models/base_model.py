...
class BaseEventModule(LightningModule):
    ...
    def __init__(
        ...
        args:
            net - core event sequences model
            metrics - metrics core object
            optimizer - optimizer for training
            sheduler - training scheduler
            head_start - number of epoch when the prediction head network starts trainins
        """
        super().__init__()
    ...
    def forward(self, batch: Tuple[torch.Tensor, torch.Tensor]) -> Any:
        """Forward pass through the model."""
        return self.net(*batch)

    def step(self, batch: Any, stage: str) -> Any:
        """General step."""
        outputs = self.forward(batch)

        if stage == "train":
            loss = self.train_metrics.compute_loss_and_add_values(
                self, batch, outputs, self.trainer.datamodule.data_process
            )
        if stage == "val":
            loss = self.val_metrics.compute_loss_and_add_values(
                self, batch, outputs, self.trainer.datamodule.data_process
            )
        if stage == "test":
            loss = self.test_metrics.compute_loss_and_add_values(
                self, batch, outputs, self.trainer.datamodule.data_process
            )
        return loss, outputs

    def training_step(self, batch: Any, batch_idx: int) -> Dict:
        """Trainig step of the model."""
