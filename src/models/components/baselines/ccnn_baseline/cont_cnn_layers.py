...
class ContConv1dSim(nn.Module):
    ...
    def forward(
        ...
        args:
            times - torch.Tensor of shape = (bs, (sim_size+1)*(max_len-1)+1), Tensor of all times (prepended with zeros by .__ad_bos)
            true_times - torch.Tensor of shape = (bs, max_len), Tensor of true times
            true_features - torch.Tensor of shape = (bs, max_len, in_channels), input tensor (aka 'encoded_outpup')
            non_pad_mask - torch.Tensor of shape = (bs, max_len),  indicates non_pad timestamps
            sim_size - int, simulated times size
