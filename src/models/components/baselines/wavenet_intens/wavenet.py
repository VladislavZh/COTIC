...
class WaveNetIntens(nn.Module):
    def __init__(
        ...
        self.position_vec_future = torch.tensor(
            [
                math.pow(10000.0, 2.0 * (i // 2) / hyperparams["hidden_size"])
                for i in range(hyperparams["hidden_size"])
            ]
        )
        ...
        self.w = nn.parameter.Parameter(
            torch.rand(num_types, hyperparams["hidden_size"])
        )
    ...
    def forward(self, event_time, event_type):
        ...
        enc_output = (
            self.leaky_relu(self.hidden_layer(x)).transpose(1, 2) * non_pad_mask
        )
    ...
    def get_lambdas(
        ...
        # future
        temp_enc = self.temporal_enc_future(
            delta_t, non_pad_mask.unsqueeze(-1).unsqueeze(-1)
        )  # shape = (bs, L, n_times, hidden_size)
