...
def apply_kernels(X, kernels):
    ...
    #X_features = np.zeros((num_examples, num_kernels * 2), dtype = np.float64) # 2 features per kernel
    X_features = torch.zeros(num_examples, num_kernels * 2, requires_grad=True)
    ...
    for i in range(num_examples):
        ...
        for j in range(num_kernels):
            ...
            ppv, max_value = apply_kernel(X[i], weights[a1:b1], lengths[j], biases[j], dilations[j], paddings[j])
            X_features[i, a2] = ppv
            X_features[i, a2+1] = max_value
