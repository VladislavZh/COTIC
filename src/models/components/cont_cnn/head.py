...
class IntensityBasedHead(nn.Module):
    ...
    def __simulate_and_compute_times_and_events(
        ...
        bos_full_times_new = torch.concat([(times[:,:-1].clone() + predicted_time.clone().detach()).unsqueeze(-1), times[:,1:].clone().unsqueeze(-1)], dim = -1).flatten(1)
        bos_full_times_new = torch.concat([torch.zeros(bs, 1).to(times.device), bos_full_times_new], dim=-1)
        all_lambda_new = pl_module.net.final(bos_full_times_new, times, enc_output, events.ne(0), 1) # shape = (bs, (num_samples + 1) * L + 1, num_types)

        predicted_event = torch.log(all_lambda_new.transpose(1,2)[:,:,1:].reshape(bs, num_types, times.shape[1]-1, 2)[...,0].transpose(1,2) + 1e-9)
