...
class WNIntensMetrics(MetricsCore):
    def __init__(self, return_time_metric, event_type_metric, sim_size=100):
    ...
    def compute_integral_unbiased(
        model, enc_output, event_time, non_pad_mask, type_mask, num_samples
    ):
        """Log-likelihood of non-events, using Monte Carlo integration."""

        diff_time = (event_time[:, 1:] - event_time[:, :-1]) * non_pad_mask[:, 1:]
        temp_time = diff_time.unsqueeze(2) * torch.rand(
            [*diff_time.size(), num_samples], device=enc_output.device
        )

        all_lambda = model.get_lambdas(
            event_time[:, :-1], enc_output[:, :-1, :], temp_time, non_pad_mask[:, 1:]
        )
    ...
    def event_and_non_event_log_likelihood(
        ...
        all_lambda = pl_module.net.get_lambdas(
            event_time[:, :-1], enc_output[:, :-1, :], temp_time, non_pad_mask[:, 1:]
        )[:, :, :, 0]
