...
class CCNNMetrics(MetricsCore):
    ...
    def __init__(
        ...
        self.type_loss_func = torch.nn.CrossEntropyLoss(
            ignore_index=-1, reduction=self.reductions["type"]
        )
    ...
    def compute_event(
        ...
        args:
            type_lambda - intensity values (at true event times) for different event types
            non_pad_mask - boolean mask encoding timestamps corresponding to the true event times

        returns:
            result - values of log-intensities at true event times
        """
    ...
    def __add_sim_times(
        ...
        # generate (random) intermediate timestamps between event times
        sim_delta_times = (
            (
        ...
        # add zeros at the beginig of the sequences
    ...
    def compute_integral_unbiased(
        ...
        """Estimate log-likelihood of non-events, using Monte Carlo integration.

        :param model: trained or training model
        :param enc_output: output of the continuous convolutions block of the model
        :param event_time: event time
        :param non_pad_mask: boolean mask encoding timestamps of true events
        :param type_mask: # Q: Do we need it here??
        :param num_samples: number of sample for MC integration

        :returns: MC-approximation of the integral term of the log-likelihood function
        """
    ...
    def event_and_non_event_log_likelihood(
        ...
        """Computes log-intensities, integral and the result log-likelihoos.

        args:
            pl_module - trained () model
            enc_output - output of the continuous convolutions block of the model
            event_time - torch.Tensor with event times
            event_type - torch.Tensor with event types

        returns:
            event_ll - log-likelihood corresponding to the true events times
            non_event_ll - integral term of the log-likelihood function
        """
        ...
        all_lambda = pl_module.net.final(
    ...
    def type_loss(self, prediction: torch.Tensor, types: torch.Tensor) -> torch.Tensor:
        """Compute event type prediction loss (cross entropy).

        args:
            prediction - predicted event types for a batch of sequences
            types - true event types for a batch of sequences

        returns:
            loss - value of event-type loss function
        """
    ...
    def time_loss(
        ...
        args:
            prediction - predicted event times for a batch of sequences
            event_time - true event time for a batch of sequences
            event_type - true event times for a batch of sequences

        returns:
            loss - value of return-time prediction loss function
        """
        prediction.squeeze_(-1)

        # get boolean mask encoding true event timestamps (not padding)
        mask = event_type.ne(0)[:, 1:]

        true = event_time[:, 1:] - event_time[:, :-1]
        prediction = prediction[:, :-1]
        loss = self.return_time_loss_func(true[mask], prediction[mask])
    ...
    def compute_loss(
        ...
        return (
            ll_loss,
            self.type_loss_coeff * type_loss + self.time_loss_coeff * time_loss,
        )
